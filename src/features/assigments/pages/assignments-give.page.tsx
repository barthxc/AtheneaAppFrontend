...
import type {
  AssignmentField,
  AssignmentGiveFormValues,
} from "@/features/assigments/types";
import { AssignmentDetails } from "../services";
...
export function AssignmentsGivePage() {
  ...
  const handleCreateAssignment = () => {
    ...
    if (!date?.from || !date?.to || !date || !member) {
      return;
    }
    ...
    // Cuando esto sea OK setDate(initialDate);
  };
      ...
      <div className="flex flex-col gap-5 items-start">
        ...
        <span>
          ...
          <p className="text-foreground uppercase font-semibold">
            {member?.name} {member?.lastName} - DNI{" "}
            {member?.identificationNumber} | Teléfono:{" "}
            {member?.phoneInfo.phone1}{" "}
            {member?.phoneInfo.phone2 && `/ ${member?.phoneInfo.phone2}`}
          </p>
        </span>
        ...
        <Form {...form}>
          <form
            ...
            className="flex flex-col gap-5 items-start w-full md:w-auto">
                    ...
                    <>
                      ...
                      {!date && (
                        <FormMessage className="text-sm">
                          Debes proveer un rango de fechas de validez para la
                          cesión
                        </FormMessage>
                      )}
                    </>
            ...
            {items.length > 0 && (
              <ul className="flex flex-col gap-2">
                ...
                {items.map((item) => (
                  <li
                    key={item.id}
                    className="flex justify-start items-center gap-2 [&>button]:hover:text-destructive">
                    <span className="text-foreground font-medium">
                      <span className="uppercase">
                        {item.itemName} - X{item.quantity} --{" "}
                        {date?.from ? date?.from.toString() : "nada"}
                      </span>
                    </span>
                    <Button
                      size="icon"
