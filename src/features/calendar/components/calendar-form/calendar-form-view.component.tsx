...
export const CalendarFormView = ({ loading, form, onSubmit }: CalendarFormViewProps) => {
				...
				<form onSubmit={form.handleSubmit(handleSubmit)} className="w-full space-y-8">
					<div className="gap-8 md:grid md:grid-cols-3 items-end">
						<FormField
							...
							name="title"
							label="Título"
							render={{ renderProp: ({ field }) => <Input disabled={loading} placeholder="Título" {...field} /> }}
						/>
						<CalendarDateRangePicker onChangeDateRage={onChangeDateRange} currentDate={date} />
						<Button disabled={loading} className="mr-auto space-y-2" type="submit">
							{action}
						</Button>
					</div>
				</form>
