...
export const Gallery = ({ className, children, ...props }: GalleryProps) => {
	return (
		<div
			className={cn("w-full grid [grid-template-columns:repeat(auto-fill,minmax(250px,1fr))] gap-5", className)}
			{...props}>
			{children}
		</div>
	);
	...
	return (
		<div className="relative">
			<img
				...
				height={height}
				alt={alt}
				className={cn("w-full object-cover rounded-lg select-none block max-w-full", className)}
			/>
			{children}
		</div>
	);
