...
export const MemberForm: React.FC<MemberFormProps> = ({ initialData, isEdit, editId }) => {
	...
	const updateMemberPaymentDate = useMemberStore((s) => s.updateMemberPaymentDate);
	...
	const onSubmit = async (data: MemberFormValues) => {
		...
		try {
			...
			const errorMessage = ErrorService.handleError(
				error.statusCode,
				isEdit ? ERROR_MESSAGES.MEMBER.CREATE_MEMBER : ERROR_MESSAGES.MEMBER.CREATE_MEMBER,
			);
			toast({
				variant: "destructive",
	...
	const onUpdatePaymentDate = async () => {
		...
		try {
			...
			if (error.statusCode) {
				const errorMessage = ErrorService.handleError(error.statusCode, ERROR_MESSAGES.MEMBER.UPDATE_PAYMENT_DATE);
				toast({
					description: errorMessage,
					variant: "destructive",
				});
		...
		<>
			...
			{isEdit && (
				<div className="flex flex-row justify-around">
					...
					<Button disabled={loading} className=" h-12 text-base" type="button" onClick={onUpdatePaymentDate}>
