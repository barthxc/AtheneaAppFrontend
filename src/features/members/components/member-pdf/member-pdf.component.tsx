...
import PdfSkeleton from "../skeletons/pdf-skeleton.component";
...
export const MemberPdf: React.FC<MemberPdfProps> = ({ id, bank }) => {
  ...
  const description = bank
    ? "Descarga del PDF del Banco de Socios"
    : "Descarga del PDF del Socio";
    ...
    const getPdfUrl = async () => {
      try {
        ...
        const errorMessage = ErrorService.handleError(
          //use error.status not error.response.StatusCode cause configure axios responseType:blob
