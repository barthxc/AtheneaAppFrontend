...
import type { MemberFormValues, UseMemberPaginationInfoProps } from "@/features/members/types";
...
const useMembersInfo = () => {
	...
	const errorMessage =
		isError && ErrorService.handleError((error as any)?.statusCode, ERROR_MESSAGES.MEMBERS_INFO.GET_STATISTICS);
...
const usePdfById = (id: string) => {
	...
	const errorMessage =
		isError && ErrorService.handleError((error as any)?.statusCode, ERROR_MESSAGES.MEMBER.GET_PDF_BY_ID);
...
const usePdfBank = () => {
	...
	const errorMessage =
		isError && ErrorService.handleError((error as any)?.statusCode, ERROR_MESSAGES.MEMBER.GET_PDF_PAYMENT_METHOD_BANK);
...
const useCreateMember = () => {
	...
	const mutation = useMutation({
		mutationFn: async (member: MemberFormValues) => await MemberService.createMember(member),
		onSuccess: () => {
		...
		onError: (error) => {
			toast({
				variant: "destructive",
				description: ErrorService.handleError((error as any)?.statusCode, ERROR_MESSAGES.MEMBER.CREATE_MEMBER),
			});
		},
	});
...
const useUpdateMember = () => {
	...
	const mutation = useMutation({
		mutationFn: ({ member, id }: { member: MemberFormValues; id: string }) => MemberService.updateMember(member, id),
		onSuccess: () => {
			toast({
		...
		onError: (error) => {
			toast({
				variant: "destructive",
				description: ErrorService.handleError((error as any)?.statusCode, ERROR_MESSAGES.MEMBER.UPDATE),
			});
		},
	});
	...
	return {
		...mutation,
		errorMessage:
			mutation.isError && ErrorService.handleError((mutation.error as any)?.statusCode, ERROR_MESSAGES.MEMBER.UPDATE),
	};
};
...
const useDeleteMember = () => {
	...
	const mutation = useMutation({
		...
		onError: (error) => {
			toast({
				variant: "destructive",
				description: ErrorService.handleError((error as any)?.statusCode, ERROR_MESSAGES.MEMBER.REMOVE),
			});
		},
	});
	...
	return {
		...mutation,
		errorMessage:
			mutation.isError && ErrorService.handleError((mutation.error as any)?.statusCode, ERROR_MESSAGES.MEMBER.REMOVE),
	};
};
...
const useMemberPaginationInfo = ({ members, hasPreviousPage, hasNextPage }: UseMemberPaginationInfoProps) => {
...
const useUpdatePaymentDate = () => {
	...
	const mutation = useMutation({
		...
		onError: (error) => {
			toast({
				variant: "destructive",
				description: ErrorService.handleError((error as any)?.statusCode, ERROR_MESSAGES.MEMBER.UPDATE_PAYMENT_DATE),
			});
		},
	});
	...
	return {
		...
		errorMessage:
			mutation.isError &&
			ErrorService.handleError((mutation.error as any)?.statusCode, ERROR_MESSAGES.MEMBER.UPDATE_PAYMENT_DATE),
	};
};
