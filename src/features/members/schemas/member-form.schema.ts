...
import { MemberIsDisabled, MemberPaymentMethod, MemberStatus, MemberStreetType } from "@/features/members/types";
...
export const memberFormSchema = z
	.object({
		...
		phoneInfo: z.object({
			...
			phone2: z.string().min(8, "El número de teléfono debe tener al menos 8 dígitos").nullable().optional(),
		}),
		...
		isDisabled: z.enum([MemberIsDisabled.si, MemberIsDisabled.no]),
		// .transform((value) => value === MemberIsDisabled.si)
		gradeDisability: z.coerce.number({ message: "Ingresa un porcentaje válido" }).nullable().optional(),
		...
		addressInfo: z.object({
			streetType: z
				...
				.nullable()
				.optional(),
			streetName: z.string().min(3, "El nombre de la calle debe tener al menos 3 caracteres").nullable().optional(),
			door: z.string().min(1, "Ingresa al menos 1 caracteres").optional(),
			block: z.string().min(1, "Ingresa al menos 1 caracteres").optional(),
			location: z.string().min(1, "Ingresa al menos 1 caracter").optional(),
			province: z.string().min(3, "El nombre de la provincia debe tener al menos 3 caracteres").nullable().optional(),
			postalCode: z.coerce.number({ message: "El código postal debe ser un número" }).nullable().optional(),
		}),
		bankInfo: z.object({
			paymentMethod: z.enum([MemberPaymentMethod.caja, MemberPaymentMethod.banco]),
			name: z.string().min(3, "El nombre debe tener al menos 3 caracteres").nullable().optional(),
		...
		observations: z.string().min(1, "Ingresa al menos 1 caracter").nullable().optional(),
	...
	.superRefine(({ isDisabled, gradeDisability }, ctx) => {
		...
		if (isDisabled === MemberIsDisabled.no && gradeDisability) {
			ctx.addIssue({
		...
		if (isDisabled === MemberIsDisabled.si) {
