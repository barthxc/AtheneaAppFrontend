...
export class MemberService {
	static createMember = async (member: MemberFormValues) => {
		try {
			const { data } = await atheneaApi.post<CreateMemberResponse>("/members", member);
	...
	static getMembersNoPay = async (params: FetchMembersParams) => {
		try {
			...
			const { data } = await atheneaApi.get<MembersResponseNew>("/members/nopay", {
				params: filteredParams,
			});
			return data;
		} catch (error: any) {
	...
	static getMembersExitu = async (params: FetchMembersParams) => {
		try {
			...
			const { data } = await atheneaApi.get<MembersResponseNew>("/members/exitu", {
				params: filteredParams,
			});
			return data;
		} catch (error: any) {
	...
	static updatePaymentDateMember = async (id: string) => {
		try {
			const { data } = await atheneaApi.patch<Members>(`/members/updatePaymentDate/${id}`);
	...
	static updateMembersInfo = async () => {
		try {
			const { data } = await atheneaApi.get<MembersInfo>("/members-info/update");
			return data;
		} catch (error: any) {
			throw error.response?.data;
		}
