...
import { Button, Form, Input, Modal, Textarea, useToast } from "@/features/core/components/ui";
...
export interface EventsContentEditProps {
	contentId: string;
	defaultValues: Pick<Content, "title" | "description">;
}

export const EventsContentEdit = ({ contentId, defaultValues }: EventsContentEditProps) => {
	...
	const handleSubmit = (data: ContentFormValues) => {
		const notModified = data.title === defaultValues.title && data.description === defaultValues.description;
		if (notModified) {
			toast({
				variant: "destructive",
	...
	return (
		<>
			...
			<Modal title="Edita el contenido" description="" isOpen={editing} onClose={closeEditModal}>
				<Form {...form}>
					<form onSubmit={form.handleSubmit(handleSubmit)} className="w-full space-y-8">
						<div className="flex flex-col gap-5">
							...
							<FormField
								...
								label="Descripción"
								render={{
									renderProp: ({ field }) => <Textarea disabled={isPending} {...field} />,
								}}
							/>
						</div>
