...
const useUpdateContent = () => {
	...
	const mutation = useMutation({
		mutationFn: ([id, content]: [string, Content]) => ContentService.updatecontent(id, content),
		onSuccess: () => {
			queryClient.invalidateQueries({
				queryKey: ContentsApiFactory.getContents._def,
			});
...
const useDeleteImageContent = () => {
	...
	const mutation = useMutation({
		mutationFn: ([contentId, imageId]: [string, string]) => ContentService.deleteImageContent(contentId, imageId),
		onSuccess: () => {
			queryClient.invalidateQueries({
				queryKey: ContentsApiFactory.getContents._def,
			});
...
const useAddImageContent = () => {
	...
	const mutation = useMutation({
		mutationFn: ([contentId, images]: [string, File[]]) => ContentService.addImageContent(contentId, images),
		onSuccess: () => {
			queryClient.invalidateQueries({
				queryKey: ContentsApiFactory.getContents._def,
			});
...
const useReorderContent = () => {
	...
	const mutation = useMutation({
		mutationFn: (contents: { id: string; position: number }[]) => ContentService.reorderContent(contents),
		onSuccess: () => {
			queryClient.invalidateQueries({
				queryKey: ContentsApiFactory.getContents._def,
			});
